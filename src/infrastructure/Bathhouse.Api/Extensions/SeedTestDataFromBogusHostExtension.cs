using Bathhouse.Api.Configuration;
...
{
  public static class SeedTestDataFromBogusHostExtension
  {
    public static void SeedTestDataFromBogus(this IHost host, ILogger logger)
    {
      ...
      using (BathhouseContext context = scope.ServiceProvider.GetRequiredService<BathhouseContext>())
      {
        ...
        var opt = new TestDataSeederOption();
        config.GetSection(TestDataSeederOption.Position).Bind(opt);
    ...
    private static List<Survey> GenerateSurveys(BathhouseContext context, Employee author, TestDataSeederOption opt)
    {
    ...
    private static List<Question> GenerateQuestions(BathhouseContext context, int count, Survey survey, TestDataSeederOption opt)
    {
    ...
    private static string GenerateOneAnswer(QuestionType type, Faker faker, TestDataSeederOption opt)
    {
      return type switch
      {
    ...
    private static List<SurveyResult> GenerateSurveyResults(BathhouseContext context, TestDataSeederOption opt, Survey survey)
    {
    ...
    private static List<Answer> GenerateAnswers(BathhouseContext context, TestDataSeederOption opt, int count, SurveyResult result, Question question)
    {
    ...
    private static Employee GenerateTechSupporter(BathhouseContext context, TestDataSeederOption opt)
    {
    ...
    private static Employee GenerateDirector(BathhouseContext context, TestDataSeederOption opt)
    {
    ...
    private static List<WorkItem> GenerateWorkItems(BathhouseContext context, TestDataSeederOption opt, Employee creator, Employee? executor = null)
    {
    ...
    private static List<Client> GenerateClients(BathhouseContext context, TestDataSeederOption opt, Office office)
    {
    ...
    private static List<Employee> GenerateManagers(BathhouseContext context, TestDataSeederOption opt)
    {
    ...
    private static List<Employee> GenerateOtherEmployees(BathhouseContext context, TestDataSeederOption opt)
    {
      var testEmployees = new Faker<Employee>(opt.Locale)
        .RuleFor(a => a.DoB, f => f.Date.Between(
          DateTime.Parse(opt.Start_birthday), 
    ...
    private static List<Office> GenerateOffices(BathhouseContext context, TestDataSeederOption opt, int count, List<Employee> employees)
    {
